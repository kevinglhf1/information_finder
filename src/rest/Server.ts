...
export default class Server {
    ...
    private static my_Insightfacade = new InsightFacade();
    ...
    public static PUT(req: restify.Request, res: restify.Response, next: restify.Next) {
        ...
        try {
            Server.my_Insightfacade.addDataset(req.params.id,body).then(function (response:InsightResponse) {
    ...
    public static DEL(req: restify.Request, res: restify.Response, next: restify.Next) {
        try {
            Server.my_Insightfacade.removeDataset(req.params.id).then(function (response:InsightResponse) {
                res.json(response.code, response.body);
            }).catch(function(err:any){
                res.json(err.code, err.body);
            });
    ...
    public static POST(req: restify.Request, res: restify.Response, next: restify.Next) {
     //   Log.trace('Server::peformQuery(..) - params: ' + JSON.stringify(req.params));
        try {
            Server.my_Insightfacade.performQuery(req.body).then(function (response:InsightResponse) {
                res.json(response.code, response.body);
            }).catch(function(err:any){
                res.json(err.code, err.body);
            });
        } catch (err) {
        //    Log.error('Server::peformQuery(..) - responding 400');
            res.json(400, {error: err.message});
        }
        return next();}
